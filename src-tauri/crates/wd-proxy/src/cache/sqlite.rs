...
impl CacheBackend for SqliteCache {
    fn get(&self, key: &str) -> CacheResult<Option<CacheEntry>> {
        ...
        let mut stmt = conn
            .prepare("SELECT key, value, created_at, ttl_ms, hit_count FROM proxy_cache WHERE key = ?")
            .map_err(|e| CacheError::Backend(e.to_string()))?;

        let mut rows = stmt.query([key]).map_err(|e| CacheError::Backend(e.to_string()))?;

        if let Some(row) = rows.next().map_err(|e| CacheError::Backend(e.to_string()))? {
            let created_ms: i64 = row.get(2).map_err(|e| CacheError::Backend(e.to_string()))?;
            let ttl_ms: Option<u64> = row.get::<_, Option<i64>>(3)
                .map_err(|e| CacheError::Backend(e.to_string()))?
                .map(|v| v as u64);
            let hit_count: u64 = row.get::<_, i64>(4)
            ...
            let entry = CacheEntry {
                key: row.get::<_, String>(0).map_err(|e| CacheError::Backend(e.to_string()))?,
                value: row.get::<_, String>(1).map_err(|e| CacheError::Backend(e.to_string()))?,
                created_at,
                ttl_ms,
                hit_count,
            };
    ...
    fn set(&self, entry: CacheEntry) -> CacheResult<()> {
        ...
        conn.execute(
            "INSERT OR REPLACE INTO proxy_cache(key, value, created_at, ttl_ms, hit_count)
             VALUES(?, ?, ?, ?, ?)",
            params![entry.key, entry.value, created_ms, ttl, entry.hit_count as i64],
        )
        .map_err(|e| CacheError::Backend(e.to_string()))?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_sqlite_evict_expired() {
        ...
        old.created_at = Utc::now() - chrono::Duration::seconds(10);
        cache.set(old).unwrap();
        cache.set(CacheEntry::new("fresh.com", "new", Some(600_000))).unwrap();
    ...
    fn test_sqlite_file_persistence() {
...

        {
            ...
            cache.set(CacheEntry::new("persist.com", "value", None)).unwrap();
        }
