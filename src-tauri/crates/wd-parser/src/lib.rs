...
/// Compiled regex: normalise colon-tab sequences.
static RE_PRE_STRING_STRIP: LazyLock<Regex> = LazyLock::new(|| Regex::new(r":\t{1,2}").unwrap());

pub fn camel_case(input: &str) -> String {
    let cleaned = RE_CAMEL_CASE.replace(input, "");
...
pub fn pre_string_strip(str: &str) -> String {
    RE_PRE_STRING_STRIP.replace_all(str, ": ").to_string()
}
...
mod tests {
    ...
    fn test_pre_string_strip_multiple() {
        assert_eq!(pre_string_strip("A:\tB\nC:\t\tD"), "A: B\nC: D");
    }
