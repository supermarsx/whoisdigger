...
mod tests {
    ...
    fn test_is_domain_available() {
        ...
        assert_eq!(
            is_domain_available("Uniregistry Query limit exceeded"),
    ...
    fn test_availability_edge_cases() {
        ...
        assert_eq!(
            is_domain_available("No match for domain but Expiration Date: 2020"),
            DomainStatus::Available
        );
        assert_eq!(
            is_domain_available("some random text with no keywords"),
            DomainStatus::Unavailable
        );
        assert_eq!(
            is_domain_available("STATUS: AVAILABLE"),
            DomainStatus::Unavailable
        );
    }
